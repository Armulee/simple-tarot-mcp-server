...
import { randomToken, sha256hex, verifyPkceS256 } from "@/lib/oauth/crypto";
...
async function handleRefreshToken(req: Request, params: URLSearchParams): Promise<Response> {
  ...
  try {
    ...
    const record = await store.consumeRefreshToken(sha256hex(refreshToken));
    if (!record || record.client_id !== clientId || record.expires_at <= Math.floor(Date.now() / 1000)) {
